...
#[test]
fn location1(){
    let text= "All friends should practice Proverbs 17:17!";
    let expected: Locations = Locations { slices: vec![(28,42)], string: text.into() }; 
    assert_eq!(expected, est::get_locations(text));
}

#[test]
fn location2(){
    let text= "This string contains no scriptures.";
    let expected: Locations = Locations { slices: vec![], string: text.into() }; 
    assert_eq!(expected, est::get_locations(text));
}